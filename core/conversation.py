...
async def _build_instructions() -> str:
    ...
    base = (
        ...
        "- If the read shows `density.web_content_visible` is true AND "
        "`density.ax_chars` is under 200, the AX tree is showing the chrome but not "
        "the real page (Notion, Linear, Slack, a browser that didn't expose its "
        "content). Skip Layer 2 and go straight to look_at_screen with the user's "
        "question — that is the cheapest path to the right answer.\n"
